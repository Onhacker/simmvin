...
Ekspor **Excel MOU** pada menu Registrasi > Cetak Desa mempertahankan kolom Kecamatan, Desa, dan Jumlah Peserta, lalu menambahkan No MOU, Jumlah Pembayaran Per Desa beserta terbilangnya, Jumlah Pembayaran Tambahan beserta terbilangnya, Total, Terbilang Total, Penandatangan, dan Jabatan Penandatangan. Penandatangan beserta jabatannya diisi otomatis dari peserta aktif yang memiliki jabatan tertinggi (Kepala, Sekretaris, Kaur, Kasi, Ketua BPD, dan seterusnya) pada registrasi desa tersebut. Pemeringkatan memakai snapshot jabatan peserta agar perubahan atau penonaktifan master tidak mengubah data historis; bila tingkat jabatannya sama, waktu pendaftaran lalu ID peserta menjadi pemutus seri. Saat desa didaftarkan, nomor MOU dialokasikan sekali dan disimpan permanen pada baris registrasi; nomor tidak dihitung ulang dari urutan ekspor. Counter berjalan per kode kabupaten/tahun dengan format `001.RAB/kode_kab/SPK/romawi_bulan/tahun`; kode kabupaten diambil dari `data_kota.kode_kota` dan underscore pada kode tidak ditampilkan di hasil mailing, sedangkan bulan/tahun memakai tanggal mulai event. Karena nomor tersimpan, penambahan desa baru, perubahan urutan kecamatan, penutupan/pembukaan event, atau ekspor dengan filter tidak mengubah nomor lama. Hasil Excel tetap diurutkan per event, kecamatan, lalu desa agar mudah dikelompokkan untuk mailing MOU. Nilai pembayaran memakai snapshot tagihan registrasi, sehingga komponen paket desa dan peserta tambahan tetap konsisten dengan transaksi yang sudah tercatat.

Untuk instalasi lama, jalankan `database/patch_registration_mou_numbers.sql` setelah `git pull`. Patch menambahkan counter, memberi nomor satu kali pada registrasi lama, menginisialisasi counter dari nomor tersimpan, lalu mengunci kolom MOU agar tidak kosong. Jalankan patch pada database transaksi MVIN (bukan database wilayah); patch aman diulang bila proses deployment perlu dicoba kembali.
