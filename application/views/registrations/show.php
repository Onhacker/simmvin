...
<div class="card card-style mt-4">
    <div class="content mb-2">
        ...
        <?php if (!$isPerParticipant && $canRecordPayment && $registration['status'] === 'active' && $remainingAmountCents > 0): ?>
            <div class="registration-village-payment-cta d-flex align-items-center rounded-s bg-green-light px-3 py-3 mt-3 mb-3"><span class="icon icon-s rounded-xl bg-green-dark color-white me-3"><i class="fa fa-wallet"></i></span><div class="min-width-zero"><p class="font-11 color-green-dark font-600 mb-n1">Pembayaran desa</p><p class="font-11 opacity-60 mb-0">Sisa <?= rupiah($remainingCommitted) ?></p></div><button type="button" class="btn btn-s bg-green-dark color-white rounded-s font-600 ms-auto" data-payment-open data-target-type="village" data-target-id="" data-target-label="<?= e($registration['village_name']) ?>" data-target-remaining="<?= e($remainingCommitted) ?>">Catat Bayar</button></div>
        <?php endif; ?>
        ...
        <?php foreach ($registration['participants'] as $participant): ?>
            ...
            ?>
            <div class="card bg-theme border rounded-s shadow-0 mb-3" data-active-participant data-participant-id="<?= (int) $participant['id'] ?>" data-participant-name="<?= e($participant['full_name']) ?>"><div class="content my-3">
                ...
                <?php if ($isPerParticipant): ?>
                    <div class="d-flex py-2 border-bottom"><span class="opacity-60">Tagihan</span><strong class="ms-auto simp-balance-value"><?= rupiah($participant['expected_amount']) ?></strong></div><div class="d-flex py-2 border-bottom"><span class="opacity-60">Terverifikasi</span><strong class="color-green-dark ms-auto simp-balance-value"><?= rupiah($participantPaid) ?></strong></div><?php if ($participantPendingCents > 0): ?><div class="d-flex py-2 border-bottom"><span class="opacity-60">Menunggu verifikasi</span><strong class="color-yellow-dark ms-auto simp-balance-value"><?= rupiah($participantPending) ?></strong></div><?php endif; ?><div class="d-flex py-2"><span class="opacity-60">Sisa</span><strong class="<?= $participantRemainingCents > 0 ? 'color-red-dark' : 'color-green-dark' ?> ms-auto simp-balance-value"><?= rupiah($participantRemaining) ?></strong></div>
                ...
                <?php if ($canManageParticipants): ?>
                    <div class="registration-participant-actions" data-participant-actions>
                        ...
                        <button type="button" class="btn btn-xxs border-red-dark color-red-dark rounded-s font-600 participant-action" data-participant-delete-open data-participant-id="<?= (int) $participant['id'] ?>" data-participant-name="<?= e($participant['full_name']) ?>" data-participant-delete-url="<?= site_url('registrasi/'.(int)$registration['id'].'/ajax/peserta/'.(int)$participant['id'].'/hapus') ?>"><i class="fa fa-trash-alt me-1"></i>Hapus</button>
                    </div>
                <?php endif; ?>
            </div></div>
        <?php endforeach; ?>
...
<?php if ($canAddParticipant): ?>
    ...
    <span id="registration-inline-payment-config" class="d-none" data-accounts="<?= e(json_encode($accountPayload, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT)) ?>" data-can-record-payment="<?= $canRecordPayment ? '1' : '0' ?>" data-billing-mode="<?= e($billingMode) ?>" data-village-id="<?= e($registration['village_id']) ?>" data-village-remaining="<?= e($remainingAmount) ?>" data-current-participant-count="<?= count($registration['participants']) ?>" data-included-participant-count="<?= (int)$registration['included_participant_count'] ?>" data-participant-fee="<?= e($registration['participant_fee']) ?>"></span>
